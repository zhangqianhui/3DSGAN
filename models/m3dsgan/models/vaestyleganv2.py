...
import numpy as np
from models.m3dsgan.models.stylegan2.op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d, conv2d_gradfix
...
class VAEGenerator(nn.Module):
    def __init__(
        ...
        if dtype_ == 'VITON':
            self.poseencoder = StyleGAN2ResnetEncoder(posech=posech, netE_num_downsampling_sp=4, spatial_code_ch=512,
                                                      netE_nc_steepness=2.0, netE_scale_capacity=1.0)
        else:
            self.poseencoder = StyleGAN2ResnetEncoder(posech=posech, netE_num_downsampling_sp=6, spatial_code_ch=512,
                                                      netE_nc_steepness=2.0, netE_scale_capacity=1.0)
    ...
    def load_weights(self):
        if self.checkpoint_path is not None:
            print('Loading pSp from checkpoint: {}'.format(self.checkpoint_path))
            ckpt = torch.load(self.checkpoint_path, map_location='cpu')
            self.generator.load_state_dict(get_keys(ckpt['g'], 'generator'), strict=True)
            self.poseencoder.load_state_dict(get_keys(ckpt['g'], 'poseencoder'), strict=True)
...
class StyleGAN2ResnetEncoder(nn.Module):
    ...
    def __init__(self, posech=64, netE_num_downsampling_sp=6, spatial_code_ch=128, netE_nc_steepness=16, netE_scale_capacity=4):
        super().__init__()
        ...
        blur_kernel = [1, 2, 1]
        self.add_module("FromRGB", ConvLayer(posech, self.nc(0), 1))
        ...
        for i in range(netE_num_downsampling_sp):
            ...
            ConvLayer(self.nc(i),
                      self.nc(i + 1), kernel_size=3,
                      blur_kernel=[1], downsample=True)
            )
...
# Style encoder
class StyleGAN2ResnetSEncoder(nn.Module):

    def __init__(self, netE_num_downsampling_gl=3, netE_nc_steepness=1, netE_scale_capacity=1.0, global_code_ch=128):
        super().__init__()
        ...
        nchannels = self.nc(netE_num_downsampling_gl)
        self.add_module(
            "ToGlobalCode",
            nn.Sequential(
    ...
    def forward(self, x):
        ...
        x = self.FromRGB(x)
        x = self.DownToGlobalCode(x)
        x = x.mean(dim=(2, 3))
        gl = self.ToGlobalCode(x)
        mu = gl[:,0:self.global_code_ch // 2]
        logvar = gl[:,self.global_code_ch // 2:]

        return [self.sample_z(mu, logvar)], mu, logvar
