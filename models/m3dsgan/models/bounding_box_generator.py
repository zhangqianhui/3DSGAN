...
from models.camera import get_rotation_matrix
...
class BoundingBoxGenerator(nn.Module):
    ...
    def get_random_offset(self, batch_size):
        ...
        r_ = r_val()
        if r_ < (self.rotation_range[1] - self.rotation_range[0]) / 2:
    ...
    def forward(self, batch_size=32):
        s, t, R, Rv2 = self.get_random_offset(batch_size)
        R = R.reshape(batch_size, self.n_boxes, 3, 3)
        Rv2 = Rv2.reshape(batch_size, self.n_boxes, 3, 3)
        return s, t, R, Rv2
