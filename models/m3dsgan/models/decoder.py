...
class Decoder(nn.Module):
    ...
    def forward(self, p_in, ray_d, z_sem=None, z_pose=None, **kwargs):

        a = F.relu
        if self.z_dim > 0:
            batch_size = p_in.shape[0]
            if z_sem is None:
                z_sem = torch.randn(batch_size, self.z_dim).to(p_in.device)
            if z_pose is None:
                z_pose = torch.rands(batch_size, self.z_dim).to(p_in.device)

        p = self.transform_points(p_in)
        net = self.fc_in(p)
        if z_pose is not None:
            net = net + self.fc_z(z_pose).unsqueeze(1)
        net = a(net)
        skip_idx = 0
        for idx, layer in enumerate(self.blocks):
            net = a(layer(net))
            if (idx + 1) in self.skips and (idx < len(self.blocks) - 1):
                net = net + self.fc_z_skips[skip_idx](z_pose).unsqueeze(1)
                net = net + self.fc_p_skips[skip_idx](p)
                skip_idx += 1

        sigma_out = self.sigma_out(net).squeeze(-1)
        net = self.feat_view(net)
        net = net + self.fc_z_view(z_sem).unsqueeze(1)
